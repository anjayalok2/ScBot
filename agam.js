...
let leonimage = fs.readFileSync("./media/wpmobile.png")
let leonimage2 = fs.readFileSync("./media/Nakano.jpg")
...
module.exports = leon = async (leon, mek) => {
	try {
		...
		const sender = mek.key.fromMe ? leon.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
        ...
        const reply = (teks) => {
	      leon.sendMessage(from, teks, text, {quoted:mek, thumbnail: leonimage})
        }
        const sendMess = (hehe, teks) => {
           leon.sendMessage(hehe, teks, text)
        }
        const mentions = (teks, memberr, id) => {
           (id == null || id == undefined || id == false) ? leon.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.png')}, extendedText, { sendEphemeral: true, contextInfo: { "mentionedJid": memberr } }) : leon.sendMessage(from, {text: teks.trim(), jpegThumbnail: fs.readFileSync('./media/wpmobile.png')}, extendedText, { sendEphemeral: true, quoted: mek, contextInfo: { "mentionedJid": memberr } })
        }
        const sendText = (from, text) => {
           leon.sendMessage(from, text, MessageType.text)
        }
        const textImg = (teks) => {
           return leon.sendMessage(from, teks, text, {quoted: mek, thumbnail: fs.readFileSync('./media/wpmobile.png')})
        }
        const ftroli = {
       ...
       const kick = function(from, orangnya){
	       ...
	       leon.groupRemove(from, [i])
        }
        }
       const kickMember = async(id, target = []) => {
           let group = await leon.groupMetadata(id)
           let owner = group.owner.replace("c.us", "s.whatsapp.net")
           let me = leon.user.jid
           for (i of target) {
           if (!i.includes(me) && !i.includes(owner)) {
           await leon.groupRemove(from, [i])
        } else {
           await leon.sendMessage(id, "Not Premited!", "conversation")
        }
    }
}
       const add = function(from, orangnya){
	       leon.groupAdd(from, orangnya)
}
      const sendBug = async(target, teks) => {
           if (!teks) teks = '.'
           await leon.relayWAMessage(leon.
           prepareMessageFromContent(target, leon.
           prepareDisappearingMessageSettingContent(0),
           {}),{waitForAck:true})
           leon.sendMessage(target, teks, 'conversation')
}
       const sendKontak = (from, nomor, nama, org = "") => {
	       const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:' + org + '\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
	       leon.sendMessage(from, {displayname: nama, vcard: vcard}, MessageType.contact, {quoted: mek})
}
      const hideTag = async function(from, text){
	       let anu = await leon.groupMetadata(from)
	       let members = anu.participants
	       let ane = []
	       for (let i of members){
	       ane.push(i.jid)
}
	       leon.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Nakano.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}  
       const sendMediaURL = async(to, url, text="", mids=[]) =>{
           ...
           leon.sendMessage(to, media, type, {quoted: mek, mimetype: mime, caption: text, thumbnail: Buffer.alloc(0), contextInfo: {"mentionedJid": mids}})
      ...
      const sendFileFromUrl = async(link, type, options) => {
	       ...
	       leon.sendMessage(from, hasil, type, options).catch(e => {
	       fetch(link).then((hasil) => {
	       leon.sendMessage(from, hasil, type, options).catch(e => {
	       leon.sendMessage(from, { url : link }, type, options).catch(e => {
          ...
          let authorname = leon.contacts[from] != undefined ? leon.contacts[from].vname || leon.contacts[from].notify : undefined	
          if (authorname != undefined) { } else { authorname = groupName }	
          function addMetadata(packname, author) {	
          if (!packname) packname = ' '; if (!author) author = ' ';author = author.replace(/[^a-zA-Z0-9]/g, '');	
       ...
       return d.toLocaleDateString(locale, { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric', hour: 'numeric', minute: 'numeric', second: 'numeric'
        ...
        if (isCmd && isGroup)
           ...
           case 'creator':
               ...
               prep = await leon.prepareMessageFromContent(from,{buttonsMessage},{})
               leon.relayWAMessage(prep)
               break     
               ///Button Text
...
kma = gam1
mhan = await leon.prepareMessage(from, kma, image)
...
kma = gam1
mhan = await leon.prepareMessage(from, kma, location)
const buttonMessages = {
...
mhan = await leon.prepareMessage(from, leonimage, image, {thumbnail: leonimage2})
const btnmenu = {
imageMessage: mhan.message.imageMessage,
contentText: `${menu}`,
footerText: `${botName}`,
...
leon.sendMessage(from, btnmenu, MessageType.buttonsMessage, {quoted: ftroli, contextInfo: {mentionedJid:[sender]}})
break
...
case 'thanks':
               ...
               buttons = [{buttonId: `mypartner`,buttonText:{displayText: 'Partner Bot'},type:1},{buttonId:`owner`,buttonText:{displayText:'Owner'},type:1}]
				               locatione = (await leon.prepareMessageFromContent(from, {"locationMessage": { "degreesLatitude": 0, "degreesLongitude": 0}}, {thumbnail:fs.readFileSync('./media/Nakano.jpg')})).message.locationMessage
		...
		for(let i of result.medias){
			if(i.url.includes('mp4')){
				let link = await getBuffer(i.url)
                    leon.sendMessage(from,link,video,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Subscribe LeonGanz`})
                } else {
                    let link = await getBuffer(i.url)
                    leon.sendMessage(from,link,image,{thumbnail: Buffer.alloc(0), quoted: mek,caption: `Subscribe LeonGanz`})                  
                }
            }
          ...
          case 'igstory': 
             ...
             .then(async result => {
              ...
              if(i.url.includes('mp4')){
              let link = await getBuffer(i.url)
              leon.sendMessage(from,link,video,{quoted: mek,caption: `Type : ${i.type}`})
              } else {
              let link = await getBuffer(i.url)
              leon.sendMessage(from,link,image,{quoted: mek,caption: `Type : ${i.type}`})                  
}
}
});
              break
...
 Durasi : ${res[0].size}`
              ...
              imageMsg = ( await leon.prepareMessage(from, fs.readFileSync(`./ytmp.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
              buttonsMessage = {footerText:'Pilih satu format di bawah ini', imageMessage: imageMsg,
              contentText:`${result}`,buttons,headerType:4}
              prep = await leon.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek})
              leon.relayWAMessage(prep)
...
case 'tiktoknowm':   
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					leon.sendMessage(from,noweem , MessageType.document, {mimetype: 'video/mp4',filename: `Tiktok Download.mp4`,quoted: mek})
					})
...
case 'tiktok':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					leon.sendMessage(from,weem , MessageType.document, {mimetype: 'video/mp4',filename: `Tiktok Wm.mp4`,quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
			break 
case 'tiktokmp3': case 'tiktokaudio':  
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					leon.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mp3',filename: `Tiktok Music.mp3`,quoted: mek})
					})
				}).catch((err) => reply(`Link tidak valid`))
			break
          case 'nhentaipdf':
          case 'nhdl':
              ...
              leon.sendMessage(from, pdf, document, { quoted: mek, mimetype: Mimetype.pdf, filename: `${get_result.result.title_romaji}.pdf`, thumbnail: ini_image })
              break
          case 'buttons1':
              await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=Nyarlathotep&q=${q}`)
		     .then(res => {
			  leon.sendMessage(from, { url: res.data.result.link }, 'audioMessage', { mimetype: 'audio/mp4', quoted: mek, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.source }}})
})
              break
          case 'buttons2':
              if (args.length < 1) return reply('Link Nya Mana?')
          ...
          case 'google':
              ...
              leon.sendMessage(from, ss, image, {caption: vars, quoted : mek, thumbnail: Buffer.alloc(0) })
              }).catch(e => {
              console.log(e)
              reply(`${e}`)
})
...
_*Tunggu Proses Upload Media......*_`
          ...
          case 'wallml':
              ...
              imageMsg = ( await leon.prepareMessage(from, fs.readFileSync(`./${sender}.jpeg`), 'imageMessage', {thumbnail: Buffer.alloc(0)})).message.imageMessage
...
anu = await fetchJson(`https://api.lolhuman.xyz/api/random/ppcouple?apikey=${setting.lolkey}`)
              ...
              imageMsg = (await leon.prepareMessageMedia(buff1, "imageMessage", { thumbnail: buff1, })).imageMessage
            ...
            buff2 = await getBuffer(anu.result.female)
              buttons = [{buttonId: `ppcp`,buttonText:{displayText: ` NEXT`},type:1}]
              imageMsg = (await leon.prepareMessageMedia(buff2, "imageMessage", { thumbnail: buff2, })).imageMessage
              buttonsMessage = {footerText:'Jangan Lupa Pjnya', imageMessage: imageMsg,
...
break
          ...
          case 'play':
              await axios.get(`https://api.zeks.xyz/api/ytplaymp3/2?apikey=Nyarlathotep&q=${q}`)
		     .then(res => {
    		  leon.sendMessage(from, '*Data berhasil didapatkan*\n\n_Silahkan tunggu, file media sedang dikirim mungkin butuh waktu beberapa menit_', text, { contextInfo: { externalAdReply: { title: res.data.result.title, body: 'Duration ' + res.data.result.duration + ', Size ' + res.data.result.size, thumbnailUrl: res.data.result.thumb, sourceUrl: res.data.result.link }}})
			  leon.sendMessage(from, { url: res.data.result.link }, 'audioMessage', { mimetype: 'audio/mp4', quoted: ftroli, contextInfo: { externalAdReply: { title: res.data.result.title, mediaType: 2, thumbnailUrl: res.data.result.thumb, mediaUrl: res.data.result.source }}})
})
              break
          case 'pinterest':
          case 'pin':
              if (args.length < 1) return reply(`${prefix}Nakano leon`)
              data = await fetchJson(`https://api.lolhuman.xyz/api/pinterest?apikey=${setting.lolkey}&query=${q}`)
              buttons = [{buttonId: `${prefix + command} ${q}`,buttonText:{displayText: `Next`},type:1}]
  ...
  if (args.length < 1) return reply('Pertanyaan apa')
					...
					leon.sendMessage(from, teks, text,{quoted:ftroli,detectLinks: false})                        
		            })              
					break
          ...
          case 'tourl':
              ...
              owgi = await leon.downloadMediaMessage(boij)
          ...
          case 'telestiker':
              ...
              leon.sendMessage(from, ini_buffer, sticker, {})
}
              break
              case 'ssweb':
...
leon.sendMessage(from, hasil, image, { thumbnail: Buffer.alloc(0), caption: `Done Jangan Lupa Subscribe LeonGanz`, quoted : mek})
})
break
          case 'attp':
              if (args.length == 0) return reply(`Example: ${prefix + command} leon`)
              buffer = await getBuffer(`https://api.xteam.xyz/attp?file&text=${encodeURI(q)}`)
              leon.sendMessage(from, buffer, sticker, { quoted: mek })
              break
          case 'sticker':
          case 'stiker':
          case 's':
          ...
          case 'sgif':
              ...
              mediat = await leon.downloadAndSaveMediaMessage(encmediat)
              ron = getRandom('.webp')
              exec(`ffmpeg -i ${mediat} -vf "scale=512:512:force_original_aspect_ratio=increase,fps=15, crop=512:512" ${ron}`, (err) => {
              fs.unlinkSync(mediat)
              if (err) return reply(`${err}`)
              exec(`webpmux -set exif ${addMetadata('leon')} ${ron} -o ${ron}`, async (error) => {
              if (error) return reply(`${error}`)
              leon.sendMessage(from, fs.readFileSync(ron), sticker, {quoted:mek})
              fs.unlinkSync(ron)
})
})
              } else if ((isMedia && mek.message.videoMessage.seconds < 11 || isQuotedVideo && mek.message.extendedTextMessage.contextInfo.quotedMessage.videoMessage.seconds < 11) && args.length == 0) {
              encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
              mediat = await leon.downloadAndSaveMediaMessage(encmedia)
              ron = getRandom('.webp')
              exec(`ffmpeg -i ${mediat} -vf "scale=512:512:force_original_aspect_ratio=increase,fps=15, crop=512:512" ${ron}`, (err) => {
              fs.unlinkSync(mediat)
              if (err) return reply(`${err}`)
          ...
          case 'tovideo':
              ...
              mediaaa = await leon.downloadAndSaveMediaMessage(encmediaaa)
              a = await webp2gifFile(mediaaa)
              mp4 = await getBuffer(a.result)
              leon.sendMessage(from, mp4, video, {mimetype: 'video/mp4', quoted: mek, caption: mess.success})
          ...
          case 'tomp3':
              ...
              leon.sendMessage(from, buffer453, audio, { mimetype: 'audio/mp4', quoted: mek })
              fs.unlinkSync(ran)
})
              } else {
              reply(mess.wrongFormat)
          ...
          case 'toimg':
              ...
              leon.sendMessage(from, buffer, image, {quoted: mek, caption: 'Nih'})
              fs.unlinkSync(ran)
})
              break
          case 'nulis':
          case 'tulis':
              ...
              leon.sendMessage(from, buff, image, {quoted: mek, caption: mess.success}).catch(e => {
...
//------------------< Ingfo Bot >-------------------
          ...
          case 'speed':
              ...
              reply(`Merespon dalam ${latensie.toFixed(4)} Sec `)
              break
              case 'bc':
					leon.updatePresence(from, Presence.composing)
					if (!isOwner && !mek.key.fromMe) return sticOwner(from)
					if (args.length < 1) return reply('Teksnya?')
					anu = await leon.chats.all()
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await leon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							leon.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						...
						buff = await leon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							leon.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
						} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
						const encmedia = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
						buff = await leon.downloadMediaMessage(encmedia)
						for (let _ of anu) {
							leon.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
						}
						reply(`Sukses mengirim Broadcast ${body.slice(4)}`)
					} else {
						for (let _ of anu) {
...
await leon.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftroli})
						}
          ...
          case 'join': 
              ...
              fak = leon.query({ json: ['action', 'invite', link],
              expect200: true })
...
if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  leon.sendMessage(from, `${oi1}`, MessageType.text)
	  }
} else if (!isQuotedSticker && !isQuotedAudio && !isQuotedImage && budy.length < 10) {
teks = leon.message.extendedTextMessage.contextInfo.quotedMessage.conversation
if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  leon.sendMessage(from, teks, MessageType.text)
	  }
} else if (isQuotedSticker) {
	encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	         median = await leon.downloadAndSaveMediaMessage(encmedian)
				anu = fs.readFileSync(median)
	if (!Number(args[0])) return reply('Jumlah harus berupa angka!')
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  leon.sendMessage(from, anu, sticker)
	  }
	...
	if (Number(args[0]) >= 50) return reply('Kebanyakan!')
	  for (let i = 0; i < args[0]; i++) {
	  leon.sendMessage(from, anu, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await leon.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Kebanyakan!')
	if (!Number(oi2)) return reply('Jumlah harus berupa angka!')
	  for (let i = 0; i < oi2; i++) {
	  leon.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
          case 'term':
          ...
          case 'leaveall':
              if (!isOwner) return  
              let totalgroup = leon.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
...
await leon.sendMessage(_.jid, btnbc, MessageType.buttonsMessage, {quoted: ftroli, contextInfo: {mentionedJid:[sender]}})
						}
						reply(`Sukses mengirim Broadcast:\n${body.slice(4)}`)
					}
					break
...
case 'tagall':
       if (!isGroupAdmins) return reply(mess.only.admin)
       if (!isGroup) return fakestatus('TAG ALL')
					members_id = []
...
//------------------< Lainnya >-------------------
          case 'getpp':
              ...
              linkpp = await leon.getProfilePicture(from) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
              buffer = await getBuffer(linkpp)
              leon.sendMessage(from, buffer, image, {caption: "Nih", quoted: mek })
              } else if (mek.message.extendedTextMessage.contextInfo.mentionedJid === null || mek.message.extendedTextMessage.contextInfo.mentionedJid === undefined) {
              mberr = mek.message.extendedTextMessage.contextInfo.participant
              linkpp = await leon.getProfilePicture(mberr) || "https://telegra.ph/file/40151a65238ba2643152d.jpg"
              buffer = await getBuffer(linkpp)
              leon.sendMessage(from, buffer, image, { quoted: mek, caption: `Profile Picture of @${mberr.split("@")[0]}`, contextInfo: { "mentionedJid": [mberr] }})
          ...
          case 'searchmsg':  //by ANU TEAM
              ...
              reply(mess.wait)
              cok = await leon.searchMessages(`${ve}`, from, batas,1) 
              if (cok.messages.length < 2) return reply('Tidak Ditemukan Pesan') 
              if (cok.messages.length < parseInt(batas)) reply(`Hanya Ditemukan ${cok.messages.length - 1} Pesan`)
              for (i=1;i < cok.messages.length;i++) {
              if (cok.messages[i].message) {
              leon.sendMessage(from, `Ditemukan!`, text, {sendEphemeral: true, quoted: cok.messages[i]}) 
}
}
              } catch (e) {
...
*Model* : ${i.phone.device_model}\n\n`
      ...
      case 'darkjokes':
				...
				leon.sendMessage(from, darkjokes, image, { quoted: mek, caption: '\`\`\`DARK JOKES\`\`\`' })
				break
				case 'caklontong':
				reply(mess.wait)
				...
				Jawaban = '\n*' + randKey.result.desc + '*'
				setTimeout(() => {
					leon.sendMessage(from, Jawaban, text, { quoted: mek })
				}, 30000)
				setTimeout(() => {
					leon.sendMessage(from, '_10 Detik lagi_', text) // ur cods
				}, 20000) // 1000 = 1s,
				setTimeout(() => {
					leon.sendMessage(from, '_20 Detik lagi_', text) // ur cods
				}, 10000) // 1000 = 1s,
				setTimeout(() => {
					leon.sendMessage(from, '_30 Detik lagi_', text) // ur cods
				}, 2500) // 1000 = 1s
				leon.sendMessage(from, Pertanyaan, text, { quoted: mek })
				break
				case 'tebakgambar':
				...
				setTimeout(() => {
					leon.sendMessage(from, '* Jawaban :* ' + randKey.result.jawaban + '\n', text, { quoted: mek }) // ur cods
				}, 30000) // 1000 = 1s,
				setTimeout(() => {
					leon.sendMessage(from, '_10 Detik lagi_', text) // ur cods
				}, 20000) // 1000 = 1s,
				...
				setTimeout(() => {
					leon.sendMessage(from, randSoal, image, { caption: '_Jelaskan Apa Maksud Gambar Ini_', quoted: mek }) // ur cods
				}, 0) // 1000 = 1s,
				break
			case 'family100':
				...
				Pertanyaan = randKey.result.soal
				setTimeout(() => {
					leon.sendMessage(from, '* Jawaban :* \n```' + randKey.result.jawaban + '```', text, { quoted: mek }) // ur cods
				}, 30000) // 1000 = 1s,
				setTimeout(() => {
					reply('_Waktu Anda Habis_')
				}, 29000)
				setTimeout(() => {
					leon.sendMessage(from, '_10 Detik lagi_', text) // ur cods
				}, 20000) // 1000 = 1s,
				setTimeout(() => {
					leon.sendMessage(from, '_20 Detik lagi_', text) // ur cods
				}, 10000) // 1000 = 1s,
				...
				setTimeout(() => {
					leon.sendMessage(from, '*' + Pertanyaan + '*', text, { quoted: mek }) // ur cods
				}, 0) // 1000 = 1s,
				break
				case 'pantun':
  ...
  rate = body.slice(1)
					...
					leon.sendMessage(from, 'Pertanyaan : *'+rate+'*\n\nJawaban : '+ te+'%', text, { quoted: mek })
					break
					case 'kapankah':
  kapankah = body.slice(1)
					const kapan =['Besok','Lusa','Tadi','4 Hari Lagi','5 Hari Lagi','6 Hari Lagi','1 Minggu Lagi','2 Minggu Lagi','3 Minggu Lagi','1 Bulan Lagi','2 Bulan Lagi','3 Bulan Lagi','4 Bulan Lagi','5 Bulan Lagi','6 Bulan Lagi','1 Tahun Lagi','2 Tahun Lagi','3 Tahun Lagi','4 Tahun Lagi','5 Tahun Lagi','6 Tahun Lagi','1 Abad lagi','3 Hari Lagi','Tidak Akan Pernah']
					const koh = kapan[Math.floor(Math.random() * kapan.length)]
					leon.sendMessage(from, 'Pertanyaan : *'+kapankah+'*\n\nJawaban : '+ koh, text, { quoted: mek })
					break
					case 'apakah':
 apakah = body.slice(1)
					const apa =['Iya','Tidak','Bisa Jadi','Coba Ulangi','Tanyakan Ayam']
					const kah = apa[Math.floor(Math.random() * apa.length)]
					leon.sendMessage(from, 'Pertanyaan : *'+apakah+'*\n\nJawaban : '+ kah, text, { quoted: mek })
					break
					case 'bisakah':
  bisakah = body.slice(1)
					const bisa =['Bisa','Tidak Bisa','Coba Ulangi','Ngimpi kah?','yakin bisa?']
					const keh = bisa[Math.floor(Math.random() * bisa.length)]
					leon.sendMessage(from, 'Pertanyaan : *'+bisakah+'*\n\nJawaban : '+ keh, text, { quoted: mek })
					break
					case 'caripesan':
...
if (cok.messages[i].message) {
leon.sendMessage(from, `Nih pesannya!`, text, {quoted: cok.messages[i]}) 
}
}
} catch(e) {
console.log(e)
...
case 'alquranaudio':
    ...
    await leon.sendMessage(from, ini_buffer, audio, { quoted: mek, mimetype: Mimetype.mp3Audio })
    break
case 'asmaulhusna':
...
if (budy.includes(`Assalamualaikum`)) {
                ...
                if (budy == 'Bot') { 
               txtt =`Hai ${pushname}, Ada Yg Bisa Saya Bntu?
Click Button Dibawah Untuk Menampilkan List Menu.`
               ...
               prep = await leon.prepareMessageFromContent(from,{buttonsMessage},{})
               leon.relayWAMessage(prep)
                    buffer = fs.readFileSync(`./media/loli`)
         leon.sendMessage(from, buffer, audio, { mimetype: 'audio/mp4', quoted: fg3, ptt: true })
                  }
...
txtt =`*Santai Lo Jamet*`
               
                
               buttons = [{buttonId: `rules`,buttonText:{displayText: 'Rules Bot'},type:1},{buttonId:`menu`,buttonText:{displayText:'Menu'},type:1}]
				               locatione = (await leon.prepareMessageFromContent(from, {"locationMessage": { "degreesLatitude": 0, "degreesLongitude": 0}}, {thumbnail:fs.readFileSync('./media/Nakano.jpg')})).message.locationMessage
...
@Yui-Chan@^3.0.0`,
               ...
               leon.relayWAMessage(prep)
                  ...
                  if (budy.includes(`Leon`)) {
                  reply(`Iya kenapa manggil crush ku?>//<`)
                  }
                  if (budy.includes(`halo`)) {
                  reply(`Hallo Juga Kak Ada Keperluan Apa, Kalau Mau Gunain Bot Ktik #menu Yakak`)
                  }
		if (budy.includes(`tq`))
if (budy.includes(`terimakasih`)) {
               txtt =`sama sama semoga harimu menyenangkan`
               }
if (budy.startsWith('=>')){
if (!isOwner) return
try {
return leon.sendMessage(from, 
